...
export function PrivateExperienceMaterials({ accessId }: { accessId: string }) {
  ...
  async function open() { setBusy(true); setError(""); try { setItems(await nativeAction("materials", { accessId })); } catch (reason) { setError(reason instanceof Error ? reason.message : "Unable to open these materials."); } finally { setBusy(false); } }
