...
function nonFacilityHref(workspace: WorkspaceId, item: NavigationItem) {
...
export function WorkspaceRoute() {
  ...
  const flatItem = workspace === "facility" ? undefined : nav.find((item) => item.slug === flatSlug);
  const routeInvalid = workspace === "facility" ? !facilityRoute : !flatItem && flatSlug.length > 0;
  const activeItem = facilityRoute?.parent ?? flatItem ?? nav[0];
  ...
  const itemHref = (item: NavigationItem) => workspace === "facility"
    ? buildFacilityHref({ parentId: item.id as FacilityParentId, programRunId: facilityProgramRunId })
    : nonFacilityHref(workspace, item);

  const activeNode = facilityRoute?.grandchild ?? facilityRoute?.child;
  ...
  return <div className="workspaceShell">
    ...
    <main className="workspaceMain">
      ...
      <div className="pageHeading"><div><p className="eyebrow">{eyebrow}</p><h1>{headingLabel}</h1><p>{headingDescription}</p></div><span className={`status ${workspace === "facility" && !routeInvalid ? "structured" : activeItem.availability}`}>{workspace === "facility" && !routeInvalid ? "Workflow structure active" : activeItem.availability === "chassis" ? "Chassis active" : activeItem.availability === "structured" ? "Workflow structured" : "Workflow planned"}</span></div>
      {routeInvalid
    ...
    <nav className="mobileNav" aria-label="Mobile workspace navigation">
      ...
      <details className="mobileMore"><summary>More</summary><div>{nav.slice(3).map((item) => <Link aria-current={activeItem.id === item.id ? "page" : undefined} className={activeItem.id === item.id ? "active" : ""} href={itemHref(item)} key={item.id}>{item.label}</Link>)}</div></details>
    </nav>
  </div>;
}
