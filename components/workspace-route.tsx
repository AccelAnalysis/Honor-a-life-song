...
export function WorkspaceRoute() {
  ...
  const flatItem = workspace === "creator" ? undefined : nav.find((item) => item.slug === flatSlug);
  const routeInvalid = workspace === "creator" ? !creatorRoute : !flatItem && flatSlug.length > 0;
  const activeItem = creatorRoute?.parent ?? flatItem ?? nav[0];
  const context = referenceContext[workspace];

  const itemHref = (item: NavigationItem) => {
    if (workspace !== "creator") return flatHref(workspace, item);
  ...
  const activeNode = creatorRoute?.grandchild ?? creatorRoute?.child;
  ...
  const structuredCreatorRoute = workspace === "creator" && !routeInvalid;
  ...
  return <div className="workspaceShell">
    ...
    <main className="workspaceMain">
      ...
      <div className="pageHeading"><div><p className="eyebrow">{eyebrow}</p><h1>{headingLabel}</h1><p>{headingDescription}</p></div><span className={`status ${structuredCreatorRoute ? styles.structuredStatus : activeItem.availability}`}>{structuredCreatorRoute ? "Workflow structure active" : activeItem.availability === "chassis" ? "Chassis active" : activeItem.availability === "structured" ? "Workflow structured" : "Workflow planned"}</span></div>
      {routeInvalid
