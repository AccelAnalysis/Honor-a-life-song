...
export const adminChildren: Record<AdminParentId, readonly AdminWorkflowNode[]> = {
  "admin-home": [
    ...
    node("dashboard-active-orders", "Active Organization Orders", "active-orders", "Active organization purchases derived from authoritative order and experience state; cancelled, declined, delivered/closed work is not counted merely because records exist.", ["orders"], { recordKind: "order" }),
    node("dashboard-active-programs", "Active Experiences", "active-programs", "Active OrganizationExperience records across the two governed experience templates.", ["programs"], { recordKind: "program" }),
  ...
  requests: [
    node("requests-new-inquiries", "New Inquiries", "new-inquiries", "Canonical Inquiry records from organization experience, consultation, partner, or sponsor entry channels.", ["requests"], { recordKind: "inquiry" }),
  ...
  programs: [
    node("programs-organization-orders", "Organization Orders", "organization-orders", "Commercial purchases owned by an organization; each completed purchase creates or links to an OrganizationExperience.", ["orders", "audit"], { recordKind: "order", action: { label: "Apply authorized order action", service: "orders" } }),
    node("programs-organization-experiences", "Organization Experiences", "organization-experiences", "OrganizationExperience records using either the group-event or full-program template without duplicating the organization workspace.", ["programs", "people", "funding"], { recordKind: "program", action: { label: "Apply authorized experience action", service: "programs" } }),
    node("programs-experience-templates", "Experience Templates", "experience-templates", "The two governed organization-owned experience templates and any future explicitly approved templates.", ["programs"], { recordKind: "program" }),
    ...
    node("programs-closed-work", "Closed Work", "closed-work", "Legitimately closed organization orders and completed experiences based on authoritative workflow state rather than inactivity.", ["orders", "programs"])
  ],
  people: [
