...
import type { OrganizationMember } from "@/domain/organization-account";
...
import { organizationRoleLabel, organizationRoleOptions, parseOrganizationRole } from "@/domain/organization-roles";
export function OrganizationTeam({ organizationId, members, canManage }: { organizationId: string; members: OrganizationMember[]; canManage: boolean }) {
  ...
  async function invite(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const item = await createOrganizationInvitation({ organizationId, email, role: parseOrganizationRole(String(values.get("role") ?? "viewer")), invitedBy: user.uid });
  ...
  return <section className={styles.surface} id="team" aria-labelledby="team-heading">
    ...
    <div className={styles.rows}>{members.map(member => <div className={styles.row} key={member.userId}><div><strong>{member.displayName}</strong><span>{member.email}</span></div><span>{organizationRoleLabel(member.role)}{member.userId === user?.uid ? " · You" : ""}</span></div>)}</div>
    {canManage ? <>
      <details className={styles.capturePanel}><summary>Invite a team member</summary><form onSubmit={invite}>
        ...
        <label><span id="team-invitation-access">Access</span><select name="role" aria-labelledby="team-invitation-access" defaultValue="viewer">{organizationRoleOptions.map(option => <option key={option.value} value={option.value}>{option.label}</option>)}</select></label>
      ...
      {invitations.map(item => <div className={styles.row} key={item.id}><div><strong>{item.email}</strong><span>Invitation pending · {organizationRoleLabel(item.role)}</span></div><div className={styles.teamActions}><button type="button" onClick={() => { setLink(`${window.location.origin}${appPath(`/accept-invitation?org=${encodeURIComponent(organizationId)}&id=${encodeURIComponent(item.id)}`)}`); setCopied(false); }}>Get link</button><button type="button" disabled={busy} onClick={() => void revoke(item.id)}>Cancel invitation</button></div></div>)}
    </> : null}
